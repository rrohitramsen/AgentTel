...
The backend MCP server provides 15 tools: `get_service_health`, `get_incident_context`, `list_remediation_actions`, `execute_remediation`, `get_recent_agent_actions`, `get_slo_report`, `get_executive_summary`, `get_trend_analysis`, `get_cross_stack_context`, `get_playbook`, `verify_remediation_effect`, `get_error_analysis`, `create_session`, `add_session_entry`, and `get_session`. The instrument agent provides 7 additional tools for instrumentation automation. See the [MCP Tools Reference](../reference/mcp-tools.md) for details on each tool.
